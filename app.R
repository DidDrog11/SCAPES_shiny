        ...
        tabItems(
            ...
            tabItem(tabName = "group_1",
                    fluidRow(
                        box(width = 12,
                            h1("Group 1"),
                            p("Group 1 villages are those with a central nucleus of housing with farmland surrounding these sites. Villages are classified through containing a built-up patch area of 0.2km²", strong("and"), "a cropland patch area of less than 0.75km²."),
...
server <- function(input, output) {
    ...
    output$group_1_params <- renderPlot({
        group_1_plot <- site_df %>%
            ...
            lapply(., function(x) {
                ...
                ggplot(x) +
                    geom_point(aes(x = value, y = group_1), alpha = 0.3, position = position_jitter(height = 0.6, width = 0, seed = 123)) +
                    labs(title = unique(x$metric),
                         y = "Group 1",
                         x = if(str_detect(unique(x$metric), "%")) "Proportion" else if(str_detect(unique(x$metric), "Area")) "Area" else "N") +
                    theme_bw() +
                    theme(legend.position = "none")
                
            })
        
        plot_grid(plotlist = group_1_plot,
                  ncol = 2)
